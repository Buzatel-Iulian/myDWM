...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			#else
			for (i = 0; i < LENGTH(blocks) - getsystraywidth(); i++)
			#endif /* INVERSED */
			{
...
getstatus(int width)
{
	...
	#endif /* INVERSED */
	{
		...
		all -= len;
		drw_text(drw, all-getsystraywidth(), 0, len, bh, 0, blockoutput[i], 0);
		/* draw delimiter */
		if (*delimiter == '\0') /* ignore no delimiter */
			continue;
		drw_setscheme(drw, scheme[SchemeNorm]);
		all -= delimlen;
		drw_text(drw, all-getsystraywidth(), 0, delimlen, bh, 0, delimiter, 0);
	}
...
updatesystray(void)
{
	...
	unsigned int sw = lrpad + systrayspacing;
